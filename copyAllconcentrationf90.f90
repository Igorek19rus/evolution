      ...
      character*100 NameMyCoord,NameConcentr,pdf_folder,fileToOpen, pathFile
	  integer pdf_type, time, sum1
...
!     READ
            ...
            open (35,file=NameConcentr)
                  do 20 jj=1,nz+1
                    read (35,799)  nv(1,jj),nv(2,jj),nv(3,jj),nv(4,jj) &
...
!      CHANGE
            ...
            do 46 iid=1,nx+1
                  do 47 jjd=1,nz+1
                      sum1=sum1+nv(iid,jjd)
47                continue
46          continue
! print 802, "---- ", t, sum1, npart
print 802, "-- ", t, sum1, npart*1., nv_all(10, 10)*1., nv(ii, jj)*1. !+ , c
            do 44 ii=1,nx+1
                  do 45 jj=1,nz+1
                        if (sum1 .gt. 0) then 
!+				nv_all(ii, jj) = nv_all(ii, jj)+nv(ii, jj)/(sum1*1.)*c/npart
				nv_all(ii, jj) = nv_all(ii, jj)+nv(ii, jj)/(sum1*1.)/npart
			endif
45                  continue
44            continue
...
!+      if (t.gt.-1 .and. t.lt.10) &
!+                  write(NameConcentr,'(A,i1,A)') &
!+                    'all_concentr/all_concentr000',t,'.dat'
!+            If(t .gt. 9 .and. t .lt. 100) &
!+                  write(NameConcentr,'(A,i2,A)') &
!+                    'all_concentr/all_concentr00',t,'.dat'
!+             If(t .gt. 99 .and. t .lt. 1000) &
!+                  write(NameConcentr,'(A,i3,A)') &
!+                    'all_concentr/all_concentr0',t,'.dat'
!+            If(t .gt. 999 .and. t .lt. 10000) &
!+                  write(NameConcentr,'(A,i4,A)') &
!+                    'all_concentr/all_concentr',t,'.dat'
!+		    NameConcentr = trim(pdf_folder)//NameConcentr
!+
!+            do 54 ii=1,nx+1
!+                  do 55 jj=1,nz+1
!+                        nvall(ii, jj) = nv_all(ii, jj)*1./nt
!+55                  continue
!+54            continue
!+
!+            open (35,file=NameConcentr)
...
12      continue
                  ...
                  write(NameConcentr,'(A,i4,A)') &
                    'all_concentr/all_concentr',ntime,'.dat'
		    NameConcentr = trim(pdf_folder)//NameConcentr

            do 54 ii=1,nx+1
                  do 55 jj=1,nz+1
                        nvall(ii, jj) = nv_all(ii, jj)*1./ntime
55                  continue
54            continue
...
40                  continue
 ...
 999      continue
	  open(676, FILE="history.txt",STATUS="old",POSITION="append")
	  write(676,*) "all concentration cancel : "//trim(pdf_folder)
	  close(676)
!     calculate a coefficient
