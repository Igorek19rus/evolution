...
c       al=cap*z(k)/(1.+cap*z(k)/.003)
	...
	if(k.ne.1.and.k.ne.n)then
	 dw2=(w2e(k+1)-w2e(k-1))/(z(k+1)-z(k-1))
	 dw3=(w3(k+1)-w3(k-1))/(z(k+1)-z(k-1))
       frmk=ee(k)/epe(k)/(2.*c7-1)*(6.*w3(k)*dw2+4.*w2e(k)*dw3)
	endif
	frm2(k)=frmk
      frm1(k)=frmk+3.*w2e(k)**2
  12  continue
      alb0=0.1*aib1/aib2
      wnn=bt*g*dt(n-2)
...
! ===============	
	...
	frm2(n)=frm2(n-1)
	 ...
	 write(13,792) zi,ws,0.,0.,0.,0.,0.,0.,0.,0.
       write(13,792) (z(k),eh(k),ep(k),w2(k),w3(k),frm1(k),ue(k),ve(k)				  ! 'nddtr.d'
     .               ,frm2(k)
c     sqrt(u(k)**2+v(k)**2)
...
! ===============	
      write(14,792) zz,heh,hepx,hw2,hw3,hfrmk,hu,hv,sqrt(hu**2+hv**2)           ! 'n15tr.dat'
     .             ,lambda
      write(15,792) z(k)/zi,e(k)*ug*ug/ws/ws,ep(k)/ws/ws*zi*ug*ug			! 'my_n15tr.dat'
     .             ,w2(k)/ws/ws,w3(k)/ws/ws/ws
     .             ,frm1(k)/ws/ws/ws/ws,u*ug/ws,v*ug/ws
